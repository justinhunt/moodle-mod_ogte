...
class utils
{
    ...
    public static function get_coverage($passage, $ignore, $listid, $listlevel)
    {
        ...
        foreach ($words as $word) {
            ...
            if ($hasmultiwordterms) {
                ...
                if ($ismultiwordterm) {
                    $wordsinterm = preg_split('/[\s]+/', $word, -1, PREG_SPLIT_NO_EMPTY);
                    $wordsintermcount = count($wordsinterm);
                }
                else {
                    $wordsintermcount = 1;
                }
            ...
            if (!empty($cleanword)) {
                ...
                if (is_array($ignores) && in_array($cleanword, $ignores)) {
                    ...
                    $ignored += $wordsintermcount;
                }
                else if (self::is_numeric_with_unit($cleanword)) {
                    ...
                    $numbers += $wordsintermcount;

                // restore new words and remove from calculation
                }
                else if ($cleanword === constants::M_FAKENEWLINE) {
                    // return an html new line (we keep \n for CodeMirror)
                    $retwords[] = "\n";
                    $worddata[] = ['word' => "\n", 'class' => '', 'data' => []];
                    $newlines += $wordsintermcount; // always 1 right?
                    ...
                    else if (array_key_exists($cleanword, $alreadyfound)) {
                    ...
                    if ($listrank === false) {
                        ...
                        if ($propernounlist) {
                            ...
                            else if (array_key_exists($cleanword, $alreadypropernoun)) {
                        ...
                        if (!empty($propernounid)) {
                            if (!$ismultiwordterm) {
                                ...
                                for ($i = 0; $i < $wordsintermcount; $i++) {
                                    $retwords[] = $wordsinterm[$i];
                                    $worddata[] = ['word' => $wordsinterm[$i], 'class' => 'mod_ogte_propernoun', 'data' => ['index' => $wordcount + $i, 'listrank' => 0]];
                                }
                            }
                            $propernouns += $wordsintermcount;
                        }
                        else {
                            if (!$ismultiwordterm) {
                                $retwords[] = $word;
                                ...
                                for ($i = 0; $i < $wordsintermcount; $i++) {
                                    $retwords[] = $wordsinterm[$i];
                                    $worddata[] = ['word' => $wordsinterm[$i], 'class' => 'mod_ogte_outoflist', 'data' => ['index' => $wordcount + $i, 'listrank' => 0]];
                                }
                            }
                            $outoflist += $wordsintermcount;
                        }
                    }
                    else {
                        ...
                        if ($listrank > $selectedlevel->top) {
                            if (!$ismultiwordterm) {
                                ...
                                for ($i = 0; $i < $wordsintermcount; $i++) {
                                    ...
                                    $retwords[] = $wordsinterm[$i];
                                    $worddata[] = ['word' => $wordsinterm[$i], 'class' => 'mod_ogte_outoflevel', 'data' => $atts];
                                }
                            }
                            $outoflevel += $wordsintermcount;
                        }
                        else {
                            if (!$ismultiwordterm) {
                                $retwords[] = $word;
                                ...
                                for ($i = 0; $i < $wordsintermcount; $i++) {
                                    $atts['index'] = $wordcount + $i;
                                    $retwords[] = $wordsinterm[$i];
                                    $worddata[] = ['word' => $wordsinterm[$i], 'class' => 'mod_ogte_inlevel', 'data' => $atts];
                                }
                            }
                            $inlevel += $wordsintermcount;
                        }
                    }
                }
                $wordcount += $wordsintermcount;
            }
            else {
                $retwords[] = $word;
        ...
        if ($wordcount <= 0) {
            ...
            foreach ($retwords as $rw) {
                ...
                else if ($rw === "\n" || str_ends_with($assembledpassage, "\n")) {
                    $assembledpassage .= $rw;
                }
                else {
                    $assembledpassage .= ' ' . $rw;
            ...
            foreach ($worddata as $wd) {
                ...
                $cleanw = trim(preg_replace('/[^\'a-zA-Z0-9 ]/', '', strip_tags($w)));
                if (empty($cleanw)) {
                    continue;
                }

                // Standardize the word to lower case for frequencies (except proper nouns if desired)
                $wlower = strtolower($cleanw);

                if (strpos($cls, 'mod_ogte_outoflist') !== false) {
                    if (!isset($outoflistfreq[$wlower])) {
                        $outoflistfreq[$wlower] = ['word' => $wlower, 'frequency' => 0];
                    }
                    $outoflistfreq[$wlower]['frequency']++;
                }

                if (strpos($cls, 'mod_ogte_outoflevel') !== false) {
                    if (!isset($outoflevelfreq[$wlower])) {
                        $outoflevelfreq[$wlower] = ['word' => $wlower, 'frequency' => 0, 'level' => (int)$wd['data']['listlevel'] + 1];
                    }
                    $outoflevelfreq[$wlower]['frequency']++;
                }

                if (strpos($cls, 'mod_ogte_ignored') !== false) {
                    if (!isset($ignoredfreq[$wlower])) {
                        $ignoredfreq[$wlower] = ['word' => $wlower, 'frequency' => 0];
                    }
                    $ignoredfreq[$wlower]['frequency']++;
                }

                if (strpos($cls, 'mod_ogte_propernoun') !== false) {
                    if (!isset($propernounfreq[$cleanw])) { // keeping original casing for proper nouns
                        $propernounfreq[$cleanw] = ['word' => $cleanw, 'frequency' => 0];
                    }
                    $propernounfreq[$cleanw]['frequency']++;
                }
            }

            // Sort arrays descending by frequency, then alphabetically
            $usortdesc = function ($a, $b) {
            ...
            $outoflistres = array_values($outoflistfreq);
            usort($outoflistres, $usortdesc);

            $outoflevelres = array_values($outoflevelfreq);
            usort($outoflevelres, $usortdesc);

            $ignoredres = array_values($ignoredfreq);
            usort($ignoredres, $usortdesc);

            $propernounres = array_values($propernounfreq);
            usort($propernounres, $usortdesc);

            // Calculate out of level summaries
            $levelcounts = [];
            foreach ($outoflevelres as $wd) {
                $lvl = $wd['level'];
                if (!isset($levelcounts[$lvl])) {
                    $levelcounts[$lvl] = 0;
                }
                $levelcounts[$lvl] += $wd['frequency'];
            }
            $levelsummary = [];
            ...
            return ['passage' => $assembledpassage,
                'worddata' => $worddata,
                'status' => 'success',
                ...
                'wordcount' => $wordcount,
                'outoflist_freq' => $outoflistres,
                'outoflevel_freq' => $outoflevelres,
                'ignored_freq' => $ignoredres,
                'propernoun_freq' => $propernounres,
                'outoflevel_summary' => $levelsummary,
    ...
    public static function is_numeric_with_unit($str)
    {
        ...
        $unit = $matches[2];
        if (\core_text::strlen($numericpart) > 0 && \core_text::strlen($unit) > 0) {
            return true;
        }
        else {
            return false;
