...
Feature: Teacher can view, comment and grade students entries
  ...
  Background:
    ...
    And I am on "Course1" course homepage
    And I follow "Test ogte name"
    And I should see "OGTE question"
    And I press "Start or edit my ogte entry"
    And I set the following fields to these values:
  ...
  Scenario: Teacher can access students entries from the ogtes list page
    When I follow "Course1"
    And I turn editing mode on
    And I add the "Activities" block
    And I click on "OGTEs" "link" in the "Activities" "block"
    Then I should see "OGTE question" in the "Test ogte name" "table_row"
    And I should see "View 2 ogte entries" in the "Test ogte name" "table_row"
    And I follow "View 2 ogte entries"

  Scenario: Teacher grades and adds/edits feedback to student's entries
    When I follow "Test ogte name"
    And I should see "OGTE question"
    And I follow "View 2 ogte entries"
    Then I should see "Student 1 first reply" in the "//table[@class='ogteuserentry m-b-1']/descendant::td[@class='userfullname'][contains(., 'Student 1')]/ancestor::table[@class='ogteuserentry m-b-1']" "xpath_element"
    And I should see "Student 2 first reply" in the "//table[@class='ogteuserentry m-b-1']/descendant::td[@class='userfullname'][contains(., 'Student 2')]/ancestor::table[@class='ogteuserentry m-b-1']" "xpath_element"
    ...
    And I follow "Test ogte name"
    And I follow "View 2 ogte entries"
    And I should see "Entry has changed since last feedback was saved" in the "//table[@class='ogteuserentry m-b-1'][contains(., 'Student 1')]" "xpath_element"
    And I should see "Student 1 edited first reply" in the "//table[@class='ogteuserentry m-b-1'][contains(., 'Student 1')]" "xpath_element"
    And I should not see "Entry has changed since last feedback was saved" in the "//table[@class='ogteuserentry m-b-1'][contains(., 'Student 2')]" "xpath_element"
    And I should see "Student 2 first reply" in the "//table[@class='ogteuserentry m-b-1'][contains(., 'Student 2')]" "xpath_element"
