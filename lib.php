 ...
 * Finds all ogte notifications that have yet to be mailed out, and mails them.
 */
function ogte_cron () {
    global $CFG, $USER, $DB;
    ...
    if ($entries = ogte_get_unmailed_graded($cutofftime)) {
        $timenow = time();
        ...
        foreach ($entries as $entry) {
            ...
            echo "Processing ogte entry $entry->id\n";
            ...
            // All cached.
            $courseogtes = get_fast_modinfo($course)->get_instances_of('ogte');
            if (empty($courseogtes) || empty($courseogtes[$entry->ogte])) {
                echo "Could not find course module for ogte id $entry->ogte\n";
                continue;
            }
            $mod = $courseogtes[$entry->ogte];
            ...
            $ogteinfo = new stdClass();
            $ogteinfo->teacher = fullname($teacher);
            $ogteinfo->ogte = format_string($entry->name, true);
            $ogteinfo->url = "$CFG->wwwroot/mod/ogte/view.php?id=$mod->id";
            $modnamepl = get_string( 'modulenameplural', 'ogte' );
            $msubject = get_string( 'mailsubject', 'ogte' );
            ...
            $posttext .= "---------------------------------------------------------------------\n";
            $posttext .= get_string("ogtemail", "ogte", $ogteinfo)."\n";
            $posttext .= "---------------------------------------------------------------------\n";
            if ($user->mailformat == 1) {  // HTML.
                $posthtml = "<p><font face=\"sans-serif\">".
                "<a href=\"$CFG->wwwroot/course/view.php?id=$course->id\">$course->shortname</a> ->".
                "<a href=\"$CFG->wwwroot/mod/ogte/index.php?id=$course->id\">ogtes</a> ->".
                "<a href=\"$CFG->wwwroot/mod/ogte/view.php?id=$mod->id\">".format_string($entry->name, true)."</a></font></p>";
                $posthtml .= "<hr /><font face=\"sans-serif\">";
                $posthtml .= "<p>".get_string("ogtemailhtml", "ogte", $ogteinfo)."</p>";
                $posthtml .= "</font><hr />";
            } else {
                $posthtml = "";
            }
            ...
            if (! email_to_user($user, $teacher, $postsubject, $posttext, $posthtml)) {
                echo "Error: OGTE cron: Could not send out mail for id $entry->id to user $user->id ($user->email)\n";
            }
            if (!$DB->set_field("ogte_entries", "mailed", "1", array("id" => $entry->id))) {
